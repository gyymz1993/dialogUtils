...
/**
 * Used by OneToucDialog while initializing the dialog. Offloads some of the code to make the main
...
public class DialogInit {
    ...
    @LayoutRes
    static int getInflateLayout(OneToucDialog.Builder builder) {
    ...
    static void init(final OneToucDialog dialog) {
        final OneToucDialog.Builder builder = dialog.builder;
...
//            dialog.downloadProgressBar.setProgressDrawable(pd);
        ...
        if (dialog.mdProgress != null) {
            dialog.mdProgress.setVisibility(builder.isShowLodaing() ? View.VISIBLE : View.GONE);
        }
...
//    dialog.neutralButton.setDefaultSelector(dialog.getButtonSelector(DialogAction.NEUTRAL, false));
        ...
        if (dialog.recyclerView != null) {
            if (builder.adapter == null) {
                ...
                if (builder.listCallbackSingleChoice != null) {
                    dialog.listType = OneToucDialog.ListType.SINGLE;
                } else if (builder.listCallbackMultiChoice != null) {
                    dialog.listType = OneToucDialog.ListType.MULTI;
                    ...
                    dialog.listType = OneToucDialog.ListType.REGULAR;
                }
                builder.adapter =
                        new DefaultRvAdapter(dialog, OneToucDialog.ListType.getLayoutForType(dialog.listType));
        ...
        if (builder.getmHeight() != 0) {
            maxHeight = (int) builder.getmHeight();
        }
    ...
    private static void setupProgressDialog(final OneToucDialog dialog) {
        final OneToucDialog.Builder builder = dialog.builder;
    ...
    private static void setupInputDialog(final OneToucDialog dialog) {
        final OneToucDialog.Builder builder = dialog.builder;
    ...
    public static void startOneToucProgressTimer(final OneToucDialog dialog, OneTouchPorgressOver mOneTouchPorgressOver, int endProgress, int speed) {
       ...
       // dialog.downloadProgress=startProgress;
        ...
        // updateProcessTimer.cancel();
        updateProcessTimer.schedule(mProgressTimerTask1, 0, speed);
    }

    private static class ProgressTimerTask1 extends TimerTask {
        public OneToucDialog dialog;
        public int endProgress;

        public ProgressTimerTask1(OneToucDialog mDialog, int mProgress) {
            this.dialog = mDialog;
            this.endProgress = mProgress;
        }
