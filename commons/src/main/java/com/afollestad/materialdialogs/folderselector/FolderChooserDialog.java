...
public class FolderChooserDialog extends DialogFragment implements OneToucDialog.ListCallback {
  ...
  public Dialog onCreateDialog(Bundle savedInstanceState) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M
      ...
      return new OneToucDialog.Builder(getActivity())
    ...
    OneToucDialog.Builder builder =
        new OneToucDialog.Builder(getActivity())
            .typeface(getBuilder().mediumFont, getBuilder().regularFont)
            .title(parentFolder.getAbsolutePath())
            .items((CharSequence[]) getContentsArray())
            .itemsCallback(this)
            .onPositive(
                new OneToucDialog.SingleButtonCallback() {
                  @Override
  ...
  private void createNewFolder() {
    new OneToucDialog.Builder(checkNotNull(getActivity(), "activity"))
        .title(getBuilder().newFolderButton)
        .input(
            0,
            0,
            false,
            new OneToucDialog.InputCallback() {
              @Override
              public void onInput(OneToucDialog dialog, CharSequence input) {
  ...
  @Override
  public void onSelection(OneToucDialog materialDialog, View view, int i, CharSequence s) {
    if (canGoUp && i == 0) {
  ...
  private void reload() {
    parentContents = listFiles();
    OneToucDialog dialog = (OneToucDialog) getDialog();
